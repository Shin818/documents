...
def diff(Beta, c):
    ...
    X = np.array([
        [np.ones(N),  np.zeros(N), c.TimeC, c.FareC],
        [np.zeros(N),  np.ones(N), c.TimeB, c.FareB],
        [np.zeros(N), np.zeros(N), c.TimeR, c.FareR]
    ])
